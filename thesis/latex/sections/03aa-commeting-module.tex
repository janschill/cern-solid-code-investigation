...
When the users are in control of their data it means they can revoke access any time, or even change their data to their liking. This means when the comment is initially created through Indico and this system's user interface it does not mean this comment needs to remain as is. Even if the interface of this system does not allow modification, a modification can still happen at the source of the storage of the comment.
...
The first improvement to this approach could be pagination. Pagination limits the number of initially loaded comments to a defined amount. This can be achieved by utilizing the date and time from the file name of the comment. Only the most recent $n$ comments by time will have to be fetched.
...
Another enhancement can be attained by caching. In its current design the comments are freshly fetched on each page load. Server-side \gls{http} caching is out of control for the clients, and complete relies on the Server implementations to do so. By Solid specification \gls{http} caching is prescribed, but not necessary \cite{solid-protocol}


The Solid server should 
...
\vspace{0.5cm}
\paragraph{Modification}\mbox{}\\

Updating the comment from data pod
    Different actions could be put in place to either improve this feature from the ecosystem architecture, or even to mitigate it.
    An improvement for this is to allow the modification but to hash the comment value and compare it to the fetched one and then render an 'updated' hint on the comment, so people to realize
