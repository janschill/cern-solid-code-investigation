...
To evaluate the security of the chosen architecture of the system it make sense to look at the \texttt{auth} package and the modules that interface the user input and the system. The user input modules can be found in the \texttt{component} package.
...
The \texttt{auth} package further holds the \texttt{SolidClient} class. The \texttt{SolidClass} is a wrapper class for the external authentication functions, which bring the authentication and session management for the Solid ecosystem.
...
Target Level: What is the desired quality level?
Current Level: What is the current level?
