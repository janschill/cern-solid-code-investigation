...
Both modules were established with the same group of people and therefore carry the same set of stakeholders. The stakeholders defined in the section of the first \gls{poc} can be found here \ref{poc1-stakeholders}.
\vspace{0.5cm}
...
The \gls{ui} for this module is rather limited as it only requires an input field for the user to provide the \gls{uri} of where to find an \gls{rdf} document that can be used to fill in the registration form, an action button and then a design for when two values on an input are probable.
