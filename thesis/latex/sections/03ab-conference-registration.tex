...
A great benefit of using Linked Data is its semantic structure and the interoperability it brings when using in systems supporting it. Indico does not make use of any common \gls{rdf} features and can therefore not extract any of those benefits. Because of this no simple way of mapping the incoming data from the data pod to the inputs exist, but a few options are feasable.

\begin{enumerate}
    \item Enrich form with some kind of descriptive indicator
    \item Parse and process existing attributes and labels
\end{enumerate}
