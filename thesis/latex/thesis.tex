\documentclass[runningheads]{thesis/latex/llncs}
\usepackage[T1]{fontenc}
\usepackage{listings}
\usepackage{graphicx}
\usepackage[inline]{enumitem}
% hide link border
\usepackage[hidelinks]{hyperref}
\usepackage{csquotes}
% abbreviations
\usepackage[acronym]{glossaries}
\makeglossaries
\include{thesis/latex/glossary}
% bibliography
\usepackage[sorting=none]{biblatex}
\addbibresource{thesis/latex/bibliography.bib}
% reset section numbering in chapters
\usepackage{bookmark}
\bookmarksetup{
  numbered,
  open
}

% BigO
\usepackage{fourier}
\DeclareMathAlphabet{\mathcal}{OMS}{cmsy}{m}{n}
\SetMathAlphabet{\mathcal}{bold}{OMS}{cmsy}{b}{n}
\newcommand{\bigO}{\mathcal{O}}

% Allow sub-figures
\usepackage{subcaption}

% \usepackage[acronym]{glossaries}
% \include{thesis/latex/glossary}
% \include{thesis/latex/terms}
% \makeglossaries


\usepackage{float}
\usepackage{upquote}
\usepackage{fancyvrb}
\usepackage{color}
\usepackage[legalpaper, margin=1in]{geometry}

% use for newline after paragraph
\newcommand{\emptybox}[2][\textwidth]{%
  \begingroup
  \setlength{\fboxsep}{-\fboxrule}%
  \noindent\framebox[#1]{\rule{0pt}{#2}}%
  \endgroup
}

% enable cell coloring
\usepackage[table]{xcolor}
% set left,right padding for table
\setlength{\tabcolsep}{10pt}
% set top,bottom padding for table
\renewcommand{\arraystretch}{1.25}

%% Save the class definition of \subparagraph
\let\llncssubparagraph\subparagraph
%% Provide a definition to \subparagraph to keep titlesec happy
\let\subparagraph\paragraph
%% Load titlesec
\usepackage[compact]{titlesec}
%% Revert \subparagraph to the llncs definition
\let\subparagraph\llncssubparagraph

% hide chapter numbering on new chapter page
\titleformat{\chapter}[display]{\normalfont\bfseries}{}{0pt}{\LARGE}

% make paragraph bold
\titleformat*{\paragraph}{\itshape\mdseries\bfseries}
% increase font size for subsection
\titleformat{\subsection}
  {\normalfont\fontsize{11}{15}\bfseries}{\thesubsection}{1em}{}

% Minted code highlighting
% \usepackage{thesis/latex/minted}

% colors for code snippets
\definecolor{commentscolor}{rgb}{0.5,0.5,0.5}
\definecolor{greenstrings}{rgb}{0,0.5,0}
\definecolor{backgroundgray}{rgb}{0.95,0.95,0.95}

\lstset{escapeinside={(*@}{@*)}}
\lstdefinelanguage{Other}%
{ sensitive=true,
  numbers=left,
  backgroundcolor = \color{backgroundgray},
  stepnumber=1,
  basicstyle=\ttfamily,
  breaklines=true,
  xleftmargin=\parindent,
  aboveskip=\bigskipamount,
  tabsize=2,
  morecomment=[l][\color{commentscolor}]{///},
  morecomment=[l][\color{commentscolor}]{//},
  morecomment=[s][\color{commentscolor}]{{(*}{*)}},
  morestring=[b]",
  showstringspaces=false,
  literate={`}{\`}1,
  stringstyle=\color{greenstrings},
  basicstyle=\small,
}

\setcounter{tocdepth}{3}
% numbering on subsubsection
\setcounter{secnumdepth}{3}

% WARNING: does not compile on GitHub Actions
% remove chapter numbers in TOC
% \usepackage{titletoc}
% \titlecontents{chapter}
%             [0em]
%             {\lsstyle\smallskip\bfseries\large}
%             {}
%             {\itshape\bfseries}
%             {\hfill\contentspage}
%             [\medskip]
% titlecontents{hsectioni}[hlefti]{habove-codei}
% {hnumbered-entry-formati}{hnumberless-entry-formati}
% {hfiller-page-formati}[hbelow-codei]

\begin{document}
\input{thesis/latex/title-page}
\newpage
\input{thesis/latex/sections/00-abstract}
\newpage
\input{thesis/latex/sections/00-acknowledgement}
\newpage
\tableofcontents
\input{thesis/latex/sections/01-introduction}
\newpage
\input{thesis/latex/sections/02-background}
\newpage
\input{thesis/latex/sections/03-investigation}
\newpage
\input{thesis/latex/sections/04-conclusion}
\newpage
% \printglossary[type=\acronymtype]
% \newpage
% \printglossary
% \newpage
\printbibliography
\end{document}
